...
# spdf_utm <- spTransform(spdf, CRS(crs(thorn_mask)@projargs))
...
coordinates(US) <- c( "long","lat")
crs(US) <- CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0")
# US <- spTransform(US, crs(thorn_mask))
US <- as.data.frame(US)
...
man <- reclassify(thorn_man,reclass_m)
man_df <- as.data.frame(man, xy= TRUE)
# man[is.na(man[])] <- 1 
...
m <- ggplot(man_df %>% na.omit()) + 
  geom_tile(aes(x=x, y=y, fill = factor(landtype_2)), alpha = .5) +#
  # facet_wrap(~ variable) +
...
wa_plot <- ggplot()+
  ...
  geom_point(data=thorn_mask, aes(x=mean(long), y=mean(lat)), 
               fill="black", size = 5, shape = 18)+wa_theme

US_plot <- ggplot()+
  geom_polygon(data=US, aes(x=long, y=lat, group=group), fill ="white",color = "gray")+
  geom_polygon(data=wa_LL, aes(x=long, y=lat, group=group), fill =grey(.85),color = "gray")+
  coord_equal()+
  theme_void()

site_map <- ggdraw() + draw_plot(m)+
  draw_plot(plot = wa_plot, -.21, -.077, scale=.33)+
  draw_plot(plot = US_plot, .2, .35, scale=.25)
        
site_map
    
ggsave(filename="D:/Dropbox/active/SD_Conservation_adoption/figures/site_map_UTM_21.10.18.jpeg",
       plot=site_map,
       width=6.5,
       height=3.5)

...
open (INPUT, "<./cli/slope_monthly_prism_ward.txt") || "Can't open file\n";
	chop($_);
	$map_list = <INPUT>;
	$tmax_list = <INPUT>;
	$tmin_list = <INPUT>;
	...
	$labels = <INPUT>;
while (<INPUT>) {
	($slope_id,$map,$ma_tmax,$ma_tmin,$elev,$pct_cover,$man_file) = split(' ', $_);
...
open (CLIMATE, "<./cli/WardCreek_daily.cli") || die("Can't open file\n");
	$line_1 = <CLIMATE>;
	$line_2 = <CLIMATE>;
	$line_3 = <CLIMATE>;
	$line_4 = <CLIMATE>;
...
print `BrLF3.exe <./run/H$slope_id.run> error.err\n`;
print `rm ds*.txt\n`;
};
